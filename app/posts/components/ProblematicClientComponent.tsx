...
export const ProblematicClientComponent: React.FC<
  ProblematicClientComponentProps
> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const fetchClientData = async () => {
      try {
        ...
        if (Math.random() < 0.2) {
          throw new Error(
            "ProblematicClientComponent: Random client-side fetch failure"
          );
        }
  ...
  const triggerRenderError = () => {
    throw new Error(
      "ProblematicClientComponent: Intentional render error triggered!"
    );
  };
