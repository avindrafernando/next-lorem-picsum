...
export const UnstableServerComponent: React.FC<
  UnstableServerComponentProps
> = async ({ children }) => {
  // ❌ MISTAKE 1: Random server-side failure without error boundary
  const random = Math.random();
  if (random < 0.15) {
    throw new Error(
      "UnstableServerComponent: Random server failure occurred! This will crash the page."
    );
  }
  ...
  const efficiency = (serverData.memoryUsage / serverData.serverLoad).toFixed(
    2
  );
    ...
    <Box>
      <Box sx={{ mb: 2, p: 1, bgcolor: "warning.light", borderRadius: 1 }}>
        <Typography variant="body2">
          ...
          <br />
          Server Load: {serverData.serverLoad.toFixed(3)} | Efficiency:{" "}
          {efficiency}
        </Typography>
      </Box>
      {children}
